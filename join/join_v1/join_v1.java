...
public class join_v1 {
    public static class joinMapper1 
		...
		if (token.contains("\"")) token = token.substring(1,token.length()-1);
                ...
                word.set(id);
                context.write(word,new IntWritable(2 * val));
    ...
    public static class joinMapper2
		...
		int score = Integer.parseInt(token);
                word.set(id);
                context.write(word,new IntWritable(2 * score + 1));
		//output.collect(word,new IntWritable(2*score + 1));
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"join_v1");
        job.setJarByClass(join_v1.class);
